...
class Test__corr_selector(unittest.TestCase):
    ...
    def test__corr_selector_matrix(self):
        ...
        self.assertEqual(
            _corr_selector(self.df_data_corr.corr(), split="above", threshold=0.15)
            .isna()
            .sum()
            .sum(),
            4,
        ...
        self.assertEqual(
            _corr_selector(self.df_data_corr.corr(), split="below", threshold=0.85)
            .isna()
            .sum()
            .sum(),
            6,
    ...
    def test__corr_selector_label(self):
        ...
        self.assertEqual(
            _corr_selector(
                self.df_data_corr.corrwith(self.target), split="above", threshold=0.2
            )
            .isna()
            .sum(),
            1,
        ...
        self.assertEqual(
            _corr_selector(
                self.df_data_corr.corrwith(self.target), split="below", threshold=0.8
            )
            .isna()
            .sum(),
            2,
