...
class Test_corr_mat(unittest.TestCase):
    ...
    def test_output_type(self):
        ...
        self.assertIsInstance(
            type(corr_mat(self.data_corr_df, target="Col1")), type(pd.io.formats.style.Styler)
        )
        self.assertIsInstance(
        ...
        self.assertIsInstance(
            type(corr_mat(self.data_corr_df, target="Col1", colored=False)), type(pd.DataFrame)
        )
        self.assertIsInstance(
            type(corr_mat(self.data_corr_df, target=self.data_corr_target_series, colored=False)),
            type(pd.DataFrame),
        )
        self.assertIsInstance(
            type(corr_mat(self.data_corr_df, target=self.data_corr_target_array, colored=False)),
            type(pd.DataFrame),
        )
        self.assertIsInstance(
            type(corr_mat(self.data_corr_df, target=self.data_corr_target_list, colored=False)),
            type(pd.DataFrame),
        )
    ...
    def test_output_shape(self):
        ...
        self.assertEqual(
            corr_mat(self.data_corr_list).data.shape[0], corr_mat(self.data_corr_list).data.shape[1]
        )
        self.assertEqual(corr_mat(self.data_corr_df, target="Col1", colored=False).shape, (3, 1))
        self.assertEqual(
            corr_mat(self.data_corr_df, target=self.data_corr_target_series, colored=False).shape, (4, 1)
        )
        self.assertEqual(
            corr_mat(self.data_corr_df, target=self.data_corr_target_array, colored=False).shape, (4, 1)
        )
        self.assertEqual(
            corr_mat(self.data_corr_df, target=self.data_corr_target_list, colored=False).shape, (4, 1)
        )
