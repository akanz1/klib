...
def convert_datatypes(
    ...
    """ Converts columns to best possible dtypes using dtypes supporting pd.NA. Temporarily not converting \
        to integers due to an issue in pandas. This is expected to be fixed in pandas 1.1. \
    ...
    category : bool, optional
        Change dtypes of columns with dtype "object" to "category". Set threshold using cat_threshold or \
        exclude columns using cat_exclude, by default True
    cat_threshold : float, optional
        Ratio of unique values below which categories are inferred and column dtype is changed to \
        categorical, by default 0.05
    ...
    for col in data.columns:
        ...
        # convert_integer does not work as expected until pandas 1.1.0 while convert_string is still
        # experimental
...
def drop_missing(
    ...
    """ Drops completely empty columns and rows by default and optionally provides flexibility to loosen \
        restrictions to drop additional non-empty columns and rows based on the fraction of NA-values.
    ...
    col_exclude : Optional[List[str]], optional
        Specify a list of columns to exclude from dropping. The excluded columns do not affect the drop \
        thresholds, by default None
...
def data_cleaning(
    ...
    drop_duplicates : bool, optional
        Drop duplicate rows, keeping the first occurence. This step comes after the dropping of missing \
    ...
    category : bool, optional
        Enable changing dtypes of "object" columns to "category". Set threshold using cat_threshold. \
        Requires convert_dtypes=True, by default True
    cat_threshold : float, optional
        Ratio of unique values below which categories are inferred and column dtype is changed to \
        categorical, by default 0.03
...
class DataCleaner(BaseEstimator, TransformerMixin):
    ...
    category: bool, default True
        Change dtypes of columns to "category". Set threshold using cat_threshold. Requires \
        convert_dtypes=True
...
def mv_col_handling(
    ...
    """ Converts columns with a high ratio of missing values into binary features and eventually drops them \
    based on their correlation with other features and the target variable. This function follows a three \
    step process:
    - 1) Identify features with a high ratio of missing values (above 'mv_threshold').
    - 2) Identify high correlations of these features among themselves and with other features in the \
    dataset (above 'corr_thresh_features').
    ...
    Note: If no target is provided, the process exits after step two and drops columns identified up to this \
    point.
    ...
    target : Optional[Union[str, pd.Series, List]], optional
        Specify target for correlation. I.e. label column to generate only the correlations between each \
        feature and the label, by default None
    mv_threshold : float, optional
        Value between 0 <= threshold <= 1. Features with a missing-value-ratio larger than mv_threshold are \
        candidates for dropping and undergo further analysis, by default 0.1
    corr_thresh_features : float, optional
        Value between 0 <= threshold <= 1. Maximum correlation a previously identified features (with a high \
        mv-ratio) is allowed to have with another feature. If this threshold is overstepped, the feature \
        undergoes further analysis, by default 0.5
    corr_thresh_target : float, optional
        Value between 0 <= threshold <= 1. Minimum required correlation of a remaining feature (i.e. feature \
        with a high mv-ratio and high correlation to another existing feature) with the target. If this \
        threshold is not met the feature is ultimately dropped, by default 0.3
...
class MVColHandler(BaseEstimator, TransformerMixin):
    ...
    mv_threshold: float, default 0.1
        Value between 0 <= threshold <= 1. Features with a missing-value-ratio larger than mv_threshold are \
        candidates  for dropping and undergo further analysis.
    corr_thresh_features: float, default 0.6
        Value between 0 <= threshold <= 1. Maximum correlation a previously identified features with a high \
        mv-ratio is allowed to have with another feature. If this threshold is overstepped, the feature \
        undergoes further analysis.
    corr_thresh_target: float, default 0.3
        Value between 0 <= threshold <= 1. Minimum required correlation of a remaining feature (i.e. feature \
        with a high mv-ratio and high correlation to another existing feature) with the target. If this \
        threshold is not met the feature is ultimately dropped.
...
def pool_duplicate_subsets(
    ...
    """ Checks for duplicates in subsets of columns and pools them. This can reduce the number of columns in \
        the data without loosing much information. Suitable columns are combined to subsets and tested for \
        duplicates. In case sufficient duplicates can be found, the respective columns are aggregated into a \
        "pooled_var" column. Identical numbers in the "pooled_var" column indicate identical information in \
        the respective rows.

        Note:  It is advised to exclude features that provide sufficient informational content by themselves \
        as well as the target column by using the "exclude" setting.
    ...
    col_dupl_thresh : float, optional
        Columns with a ratio of duplicates higher than "col_dupl_thresh" are considered in the further \
        analysis. Columns with a lower ratio are not considered for pooling, by default 0.2
    subset_thresh : float, optional
        The first subset with a duplicate threshold higher than "subset_thresh" is chosen and aggregated. If \
        no subset reaches the threshold, the algorithm continues with continuously smaller subsets until \
        "min_col_pool" is reached, by default 0.2
    min_col_pool : int, optional
        Minimum number of columns to pool. The algorithm attempts to combine as many columns as possible to \
        suitable subsets and stops when "min_col_pool" is reached, by default 3
    exclude : Optional[List[str]], optional
        List of column names to be excluded from the analysis. These columns are passed through without \
        modification, by default None
    return_details : bool, optional
        Provdies flexibility to return intermediary results, by default False
...
class SubsetPooler(BaseEstimator, TransformerMixin):
    """ Wrapper for pool_duplicate_subsets(). Allows pool_duplicate_subsets() to be put into a pipeline with \
        similar functions (e.g. using DataCleaner() or MVColHandler()).

    Parameters
    ----------
    col_dupl_ratio: float, default 0.2
        Columns with a ratio of duplicates higher than "col_dupl_ratio" are considered in the further \
        analysis. Columns with a lower ratio are not considered for pooling.
    dupl_thresh: float, default 0.2
        The first subset with a duplicate threshold higher than "dupl_thresh" is chosen and aggregated. If \
        no subset reaches the threshold, the algorithm continues with continuously smaller subsets until \
        "min_col_pool" is reached.
    min_col_pool: integer, default 3
        Minimum number of columns to pool. The algorithm attempts to combine as many columns as possible to \
        suitable subsets and stops when "min_col_pool" is reached.
